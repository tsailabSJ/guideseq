...
def edlibAlign(gRNA,window_seq,window_seq_revcomp,max_score=7):
	...
	if strand != None:
		...
		if strand == "+" and N_indel>0 and alignment['N_mis']<=4:
			# try no gap allowed alignment
			regex_out = gapless_alignment(gRNA,window_seq)
		elif strand == "-" and N_indel>0 and alignment['N_mis']<=4:
			# try no gap allowed alignment
		...
		if regex_out!=None:
			...
			alignment['query_aligned'] = regex_out.group()
			alignment['target_aligned'] = gRNA
...
use_cols=['#chr','end',"nuclease.rev.plus","nuclease.fwd.plus","nuclease.fwd.minus","nuclease.rev.minus","control.rev.plus","control.fwd.plus","control.fwd.minus","control.rev.minus","nuclease.mispriming.plus","nuclease.mispriming.minus","control.mispriming.plus","control.mispriming.minus"]
...
df_list = Parallel(n_jobs=ncore,verbose=10)(delayed(off_target_assignment)(dfs[i],target_list,flank,max_score) for i in range(ncore))
...
df2.columns = df.columns[:15].tolist()+["start_absolute","end_absolute","query_aligned","target_aligned","strand","editDistance","N_mis","N_ins","N_del","cas9_cut_position","on_target_sequence"]
...
# df4 = df3.sort_values(['#chr','start','distance_between_cas9_cut_and_most_frequent_guideseq_read_start'])

df3.to_csv(f"{file}.identified.tsv",sep="\t",index=False)
